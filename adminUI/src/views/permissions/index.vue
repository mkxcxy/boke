        ...
        <el-table
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    <img v-if="!!scope.row.userImage" :src="scope.row.userImage" alt="用户头像">
                    <img v-else :src="scope.row.userImage" alt="用户头像">
                </template>
            </el-table-column>
            <el-table-column
                    label="用户名"
                    width="180">
                <template slot-scope="scope">
                    <span style="margin-left: 10px">{{ scope.row.userName }}</span><span class="pl-1"
                                                                                         :class="{ 'text--blue':scope.row.sex,'text--pink':!scope.row.sex }">{{ scope.row.sex?'♂':'♀' }}</span>
                </template>
            </el-table-column>
            <el-table-column
                    label="权限"
                    width="180">
                <template slot-scope="scope">
                    <span :class="{ 'text--blue':scope.row.permissions.name=='管理员','text--pink':scope.row.permissions.name!='管理员' }">{{ scope.row.permissions.name }}</span>
                </template>
            </el-table-column>
            ...
            >
                <template slot-scope="scope">
                    <el-button
                            size="mini"
                            :disabled="scope.row.permissions.name=='管理员'"
...
<script>
    import {getPrmissions, getUsers, userHandle} from "../../api/user";
        ...
        methods: {
            ...
            handleDelete: function (id) {
                ...
                this.$confirm('此操作将永久删除该用户, 是否继续?', '提示', {
                    ...
                    userHandle({_id: id},'delete').then(res => {
                        that.$message.success(res.message);
                        that.refresh();
                    });
                }).catch(() => {
